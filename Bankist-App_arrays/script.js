...
const checkDogs = function (dogsJulia, dogsKate) {
  const dogsJuliaNew = dogsJulia.slice().slice(1, -2);

  const allDogsAge = [...dogsJuliaNew, ...dogsKate];

  allDogsAge.forEach((dogAge, i) => {
    const adultOrPuppy = dogAge >= 3 ? 'an adult' : 'a puppy';
    console.log(
      `Dog number ${i + 1} is ${adultOrPuppy}, and is ${dogAge} years old`
    );
  });
};
...
const obj = dogs.find(dog => {
  return dog.owners.find(owner => owner === 'Sarah');
});
const food =
  obj.recFood > obj.curFood
    ? 'eating too much'
    : obj.recFood < obj.curFood
    ? 'eating too little'
...
const ownersTooMuch = dogs.flatMap(obj => {
  return obj.recFood > obj.curFood ? obj.owners : [];
});
// console.log(ownersTooMuch);

const ownersTooLittle = dogs.flatMap(obj => {
  return obj.recFood < obj.curFood ? obj.owners : [];
});
// console.log(ownersTooLittle);

// 4. Log a string to the console for each array created in 3., like this: "Matilda and Alice and Bob's dogs eat too much!" and "Sarah and John and Michael's dogs eat too little!"
// const sentence = ownersTooMuch.reduce(
//   (acc, word) => `${acc} and ${word}'s dogs eat too much`
// );
// console.log(sentence);

const sentence = ownersTooMuch.reduce((acc, owner, index) => {
  if (index === ownersTooMuch.length - 1) {
    return acc + `${owner}'s dogs eat too much!`;
  } else {
    return acc + owner + ' and ';
  }
}, '');

// console.log(sentence);

const sentence2 = ownersTooLittle.reduce((acc, owner, index) => {
  if (index === ownersTooLittle.length - 1) {
    return acc + `${owner}'s dogs eat too little!`;
  } else {
    return acc + owner + ' and ';
  }
}, '');
// console.log(sentence2);
// Output: "Michael and Alice and Bob's dogs eat too much!"
